...
Burokku uses one thread-affine QuickJS runtime and one live DOM on the native UI thread. The platform event loop owns the process main thread and drives a bounded Tokio tick plus one persistent `LocalSet`. JavaScript mutations complete before `about_to_wait`; layout and owned scene construction borrow the live DOM only for the frame-building scope, then release it before native or GPU presentation work.
