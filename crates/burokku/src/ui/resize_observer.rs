...
//!
//! Layout computation publishes snapshots; consumers explicitly take records.
//! This module neither invokes callbacks nor requires a JavaScript context.
...
pub enum ResizeObserverError {
    #[error("the observer's document has been dropped")]
    Closed,
    #[error(transparent)]
...
struct RegistryState {
    observers: SlotMap<ObserverId, Vec<Observation>>,
    generation: u64,
...
impl ResizeObserverRegistry {
    pub(crate) fn create(&self) -> ResizeObserver {
        let id = self.0.borrow_mut().observers.insert(Vec::new());
        ResizeObserver {
    ...
    pub(crate) fn publish(&self, layout: Rc<ComputedLayout>) {
        self.0.borrow_mut().latest = Some(layout);
    }
    ...
    pub(crate) fn retained_targets(&self) -> Vec<NodeId> {
        self.0
            ...
            .observers
            .values()
            .flat_map(|observations| observations.iter().map(|o| o.target))
            .collect()
    }

    pub(crate) fn prune(&self, dom: &Dom) {
        for observations in self.0.borrow_mut().observers.values_mut() {
            observations.retain(|o| dom.contains(o.target));
        }
    }
}
...
impl Drop for ResizeObserver {
    fn drop(&mut self) {
        if let Some(registry) = self.registry.upgrade() {
            registry.borrow_mut().observers.remove(self.id);
        }
    }
}
...
impl ResizeObserver {
    ...
    pub fn observe(&self, dom: &Dom, target: NodeId) -> Result<(), ResizeObserverError> {
        ...
        observations.retain(|o| o.target != target);
    ...
    pub fn unobserve(&self, target: NodeId) {
        if let Some(registry) = self.registry.upgrade() {
            if let Some(observations) = registry.borrow_mut().observers.get_mut(self.id) {
                observations.retain(|o| o.target != target);
            }
        }
    }
    ...
    pub(crate) fn commit(
        ...
        if batch.observer != self.id
            || batch.layout.revision() != dom.revision()
            || !state
                .latest
                .as_ref()
