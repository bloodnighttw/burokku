...
fn nested_flex_items_retain_final_text_geometry() {
    ...
    let small = document.create_node(ElementKind::Text("aligned through Glyphon metrics".into()));
...
fn block_baseline_comes_from_the_first_eligible_descendant() {
    ...
    assert!(
        (first_layout.y + first_baseline - reference_layout.y - reference_baseline).abs() < 0.01
    );
}
