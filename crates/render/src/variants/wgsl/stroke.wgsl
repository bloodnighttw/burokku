...
struct VertexOutput {
    ...
    @location(4) @interpolate(flat) width: f32,
};
...
fn fragment_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let inner_size = input.size - vec2<f32>(input.width * 2.0);
    if all(inner_size > vec2<f32>(0.0)) {
        let inner_position = input.local_position - vec2<f32>(input.width);
        let inner_corners = max(input.corners - vec4<f32>(input.width), vec4<f32>(0.0));
        if rounded_distance(inner_position, inner_size, inner_corners) <= 0.0 {
            discard;
        }
    }
