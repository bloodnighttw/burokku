...
impl WinitView {
    pub(super) fn new(
        ...
        // Match the Metal surface setup used by WGPU 0.19: install the
        // CAMetalLayer as the NSView's root backing layer so newer WGPU reuses
        // it instead of creating an observer-managed child layer.
        let metal_layer = unsafe { CAMetalLayer::new() };
        unsafe {
            this.setLayer(Some(&metal_layer));
        }
        this.setWantsLayer(true);
        metal_layer.setContentsGravity(unsafe { kCAGravityTopLeft });
